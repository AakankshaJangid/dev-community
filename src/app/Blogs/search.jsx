        ...
        <Sidebar/>
          <div>
            ...
            <div className='flex relative top-16 left-10'>
              ...
              <div>
              <input  type="text" placeholder='Search communities' className='bg-slate-600 outline-none rounded-lg px-52 py-3'   value={searchInput}
              ...
              {filteredProfiles.map((profile, index) => (
                <div className='flex gap-72' key={index}>
                  <div className='flex gap-6'>
                    <Image src='/user-3-fill.png' height={15} width={30} alt='user' />
                    <h1 className='relative top-2'>{profile}</h1>
                  </div>
                  <button className='bg-indigo-700 text-white px-6 py-1 rounded-xl hover:bg-indigo-600'>View</button>
                </div>
              ))}
            </div>
          )}
